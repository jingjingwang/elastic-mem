...
bool PSScavenge::invoke() {
  ...
  size_t preyoungsize = young_gen->capacity_in_bytes();
  const bool scavenge_done = PSScavenge::invoke_no_policy();
  if (GCTriggerMode == 1) young_gen->shrink_rss(preyoungsize);
  bool need_full_gc = !scavenge_done;
  if (GCTriggerMode == 0)
    need_full_gc |= policy->should_full_GC(heap->old_gen()->free_in_bytes());
...
bool PSScavenge::invoke_no_policy() {
  ...
  if (!should_attempt_scavenge()) {
    if (GCTriggerMode == 0) return false;
  }
...

  {
    ...
    if (promotion_failure_occurred) {
      heap->externalGCStats->pmfailure = true;
       ...
       if (PrintTenuringDistribution) {
        ...
        if (UseAdaptiveGenerationSizePolicyAtMinorCollection &&
          ...
          size_policy->check_gc_overhead_limit(young_live,
                                               eden_live,
                                               ElasticMem ? old_gen->gen_size_limit() : old_gen->max_gen_size(), 
                                               max_eden_size,
                                               false /* not full gc*/,
                                               gc_cause,
    ...
    if (PrintGC) {
      if (PrintGCDetails) {
        ...
        old_gen->print_used_change(old_gen_used_before);
      }
