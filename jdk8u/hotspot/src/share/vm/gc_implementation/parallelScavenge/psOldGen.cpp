...
void PSOldGen::initialize_virtual_space(ReservedSpace rs, size_t alignment) {
  ...
  if (!_virtual_space->expand_by(ElasticMem ? _max_gen_size : _init_gen_size)) {
...
bool PSOldGen::expand_by(size_t bytes) {
 ...
 if (!ElasticMem) {
  ...
  result = resize_object_space(capacity_in_bytes() + bytes);
  ParallelScavengeHeap* heap = (ParallelScavengeHeap*)Universe::heap();
  heap->externalGCStats->expand_count += 1;
  heap->externalGCStats->expand_size += bytes;
 }
...
void PSOldGen::shrink_rss(size_t presize) {
...
void PSOldGen::shrink(size_t bytes) {
  ...
  if (size > 0) {
   ...
   if (!ElasticMem) {
    ...
    resize_object_space(capacity_in_bytes() - bytes);
   }
...
bool PSOldGen::resize_object_space(size_t newsize) {
  if (newsize > virtual_space()->max_size()) {
    return false;
  }
  HeapWord* low = (HeapWord*)virtual_space()->low();
  HeapWord* high = low + newsize / HeapWordSize;
  MemRegion new_memregion(low, high);
  start_array()->set_covered_region(new_memregion);
  Universe::heap()->barrier_set()->resize_covered_region(new_memregion);
  object_space()->initialize(new_memregion, SpaceDecorator::DontClear, SpaceDecorator::DontMangle);
  return true;
}
