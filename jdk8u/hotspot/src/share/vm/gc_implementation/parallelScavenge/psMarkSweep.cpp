...
void PSMarkSweep::invoke(bool maximum_heap_compaction, bool expect_pm, size_t move_unit) {
  ...
  size_t youngpresize = young_gen->capacity_in_bytes();
  size_t oldpresize = old_gen->capacity_in_bytes();

  if (ScavengeBeforeFullGC && !expect_pm) {
  ...
  if (ScavengeAfterFullGC && expect_pm) {
    size_t movesize = (young_gen->eden_space()->capacity_in_bytes() - young_gen->eden_space()->used_in_bytes()) / move_unit * move_unit;
    size_t youngsize = young_gen->eden_space()->capacity_in_bytes() - movesize;
    size_t oldsize = old_gen->capacity_in_bytes() + movesize;
    if (young_gen->eden_space()->used_in_bytes() <= oldsize - old_gen->used_in_bytes()) {
      young_gen->virtual_space()->_realtime_max_size = youngsize; // + 2 * survivor_size;
      old_gen->virtual_space()->_realtime_max_size = oldsize;
      heap->resize_young_gen(youngsize, 0);
      heap->resize_old_gen(oldsize - old_gen->used_in_bytes()); 
      PSScavenge::invoke_no_policy();
    }
  }

  if (GCTriggerMode == 1 && heap->doneinit) {
    young_gen->shrink_rss(youngpresize);
    old_gen->shrink_rss(oldpresize);
  }
}
    ...
    if (UseAdaptiveSizePolicy) {
      ...
      if (UseAdaptiveGenerationSizePolicyAtMajorCollection &&
        ...
        size_policy->compute_generations_free_space(young_live,
                                                    ...
                                                    old_live,
                                                    cur_eden,
                                                    ElasticMem ? old_gen->gen_size_limit() : max_old_gen_size,
                                                    max_eden_size,
                                                    true /* full gc*/);
