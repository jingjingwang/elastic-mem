...
jint ParallelScavengeHeap::initialize() {
  ...
  adjustment_mutex = new Monitor(Mutex::event, "HeapAdjustmentMutex", false);
  doneinit = false;
  ...
  if (JVMPort > 0) {
    externalGCStats = new ExternalGCStats();
    ListenThread* listen_thread = new ListenThread();
    int ret = os::create_thread(listen_thread, os::listen_thread);
    assert(ret, "os create listen thread failed");
    os::start_thread(listen_thread);
  }
  return JNI_OK;
}
...
HeapWord* ParallelScavengeHeap::mem_allocate(
  ...
  *gc_overhead_limit_was_exceeded = false;
  ...
  while (result == NULL) {
    ...
    // total_collections() value!
    {
      ...
      if (GC_locker::is_active_and_needs_gc()) {
        ...
        if (!jthr->in_critical()) {
          ...
          if (GCTriggerMode == 0 || doneinit == false) return NULL;
        }
      }
    }

    if (result == NULL) {
     if (GCTriggerMode == 0 || doneinit == false) {
    ...
    // 0: internal gc, 1: external gc & sleep, 2: external gc & crash
    if (GCTriggerMode == 2) exit(1);
    if (GCTriggerMode == 1) {
       struct timeval t1, t2;
       gettimeofday(&t1, NULL);
       {
         MutexLocker ml(this->adjustment_mutex);
         this->adjustment_mutex->wait(true); // not sure
       }
       gettimeofday(&t2, NULL);
       externalGCStats->null_alloc_count ++;
       externalGCStats->null_alloc_sleep_time += t2.tv_sec*1000.0+t2.tv_usec/1000.0 - t1.tv_sec*1000.0-t1.tv_usec/1000.0;
    }
