...
PSYoungGen::PSYoungGen(size_t        initial_size,
  ...
  _min_gen_size(ElasticMem ? 0 : min_size),
...
void PSYoungGen::initialize_virtual_space(ReservedSpace rs, size_t alignment) {
  ...
  if (!virtual_space()->expand_by(ElasticMem ? _max_gen_size : _init_gen_size)) {
    vm_exit_during_initialization("Could not reserve enough space for "
                                  "object heap");
  }
  if (ElasticMem) _virtual_space->_realtime_max_size = _init_gen_size;
}
...
void PSYoungGen::compute_initial_space_boundaries() {
  ...
  // Compute sizes
  size_t alignment = heap->space_alignment();
  size_t size = ElasticMem ? _init_gen_size : virtual_space()->committed_size();
...
void PSYoungGen::set_space_boundaries(size_t eden_size, size_t survivor_size) {
  ...
  char *to_start, *from_start, *to_end, *from_end, *eden_start, *eden_end;
  if (ElasticMem) {
    eden_start = virtual_space()->low();
    eden_end = eden_start + eden_size;
    to_start = eden_start + _max_eden_size; // + alignment;
    to_end = to_start + survivor_size;
    from_start = to_start + _max_survivor_size; // + alignment;
    from_end = from_start + survivor_size;
  } else {
    eden_start = virtual_space()->low();
    to_start   = eden_start + eden_size;
    from_start = to_start   + survivor_size;
    from_end   = from_start + survivor_size;
...
size_t PSYoungGen::max_size() const {
  if (ElasticMem) return virtual_space()->max_size();
  return _reserved.byte_size();
}

size_t PSYoungGen::gen_size_limit() const {
  if (ElasticMem) return MIN2(virtual_space()->max_size(), _max_gen_size);
...
void PSYoungGen::shrink_rss(size_t presize) {
  virtual_space()->shrink_rss_to(presize);
}
...
bool PSYoungGen::resize_generation(size_t eden_size, size_t survivor_size) {
  ...
  if (desired_size > orig_size) {
    ...
    if (!ElasticMem) {
      if (!virtual_space()->expand_by(change)) return false;
    }
    else {
      ParallelScavengeHeap* heap = (ParallelScavengeHeap*)Universe::heap();
      heap->externalGCStats->expand_count += 1;
      heap->externalGCStats->expand_size += change;
    }
    if (ZapUnusedHeapArea) {
      // Mangle newly committed space immediately because it
      // can be done here more simply that after the new
...
void PSYoungGen::resize_spaces(size_t requested_eden_size,
  ...
  if (eden_from_to_order) {
    ...
    if (maintain_minimum) {
      ...
      if (ElasticMem) {
        eden_size = MIN2(eden_size, _max_eden_size);
      }
    }
    ...
    if (ElasticMem) {
      ...
      to_end   = MIN2(to_end, (char*)virtual_space()->high());
      from_end = from_start + requested_survivor_size;
      from_end   = MIN2(from_end, to_start);
    } else {
    ...
    if (ElasticMem) {
      ...
      from_end = from_start + requested_survivor_size;
      from_end   = MIN2(from_end, (char*)virtual_space()->high());
    } else {
  ...
  eden_space()->initialize(edenMR,
                           ElasticMem ? SpaceDecorator::DontClear : SpaceDecorator::Clear,
