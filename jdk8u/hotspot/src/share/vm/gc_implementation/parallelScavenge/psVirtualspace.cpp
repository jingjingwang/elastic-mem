...
bool PSVirtualSpace::expand_by(size_t bytes) {
  ...
  if (ElasticMem) {
    if (_realtime_max_size > 0 && committed_size() + bytes > _realtime_max_size)
      return false;
  }
...
void PSVirtualSpace::shrink_rss_to(size_t prev_cap) {
  if (prev_cap <= this->_realtime_max_size) return;
  size_t bytes = prev_cap - this->_realtime_max_size;
  assert(is_aligned(bytes), "arg not aligned");
  char* const base_addr = committed_low_addr() + this->_realtime_max_size;
  mmap(base_addr, bytes, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS|MAP_NORESERVE|MAP_FIXED, -1, 0);  
  ParallelScavengeHeap* heap = (ParallelScavengeHeap*)Universe::heap();
  heap->externalGCStats->shrink_rss_count += 1;
  heap->externalGCStats->shrink_rss_size += bytes;
}
