...
class PSVirtualSpace : public CHeapObj<mtGC> {
 ...
 public:
  ...
  virtual void   shrink_rss_to(size_t prev_cap);
...
inline size_t PSVirtualSpace::max_size() const {
  if (!ElasticMem) ShouldNotReachHere();
  size_t max_size = reserved_size();
  if (_realtime_max_size > 0)
    max_size = MIN2(_realtime_max_size, max_size);
  return max_size;
}
