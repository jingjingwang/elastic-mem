...
bool Arguments::check_vm_args_consistency() {
  ...
  if (UseParallelOldGC && ParallelOldGCSplitALot) {
    ...
    if (!FLAG_IS_CMDLINE(ScavengeAfterFullGC)) {
      FLAG_SET_CMDLINE(bool, ScavengeAfterFullGC, false);
    }
  }
...
jint Arguments::parse_each_vm_init_arg(const JavaVMInitArgs* args,
  ...
  for (int index = 0; index < args->nOptions; index++) {
    ...
    // Xmaxf
    } else if (match_option(option, "-JVMPort=", &tail)) {
      int jvmport = (int)(atof(tail));
      if (jvmport < 0 || jvmport > 65535) {
        jio_fprintf(defaultStream::error_stream(), "Bad JVM port number\n");
        return JNI_EINVAL;
      }
      FLAG_SET_CMDLINE(uintx, JVMPort, (uintx)jvmport);
    } else if (match_option(option, "-GCTriggerMode", &tail)) {
      int mode = (int)(atof(tail));
      FLAG_SET_CMDLINE(uintx, GCTriggerMode, (uintx)mode);
    } else if (match_option(option, "-ElasticMem", &tail)) {
      FLAG_SET_CMDLINE(bool, ElasticMem, true);
